...
public class main {
    ...
    public static boolean validaEsp(Personagem per){
         if(per instanceof Mago){
           if(((Mago) per).getMana()==3)return true;
           return false;
        }
         return false;
    }
        ...
        for(int i=0;i<tabu.descobreTabu(tabu).length;i++){
            for(int j=0;j<tabu.descobreTabu(tabu).length;j++){
                if(tabu.descobreTabu(tabu)[i][j]!=null){
                    if(Integer.parseInt(tabu.descobreTabu(tabu)[i][j].visual.substring(1))==1) cont1++;
                    else if(Integer.parseInt(tabu.descobreTabu(tabu)[i][j].visual.substring(1))==2) cont2++;
                }
            }
        }
        if(cont1==0 || cont2==0){
